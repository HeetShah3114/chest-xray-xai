...
Evaluates a DenseNet variant (base, imagenet, or attention) on the CheXpert small validation dataset.
...
Usage:
    Run the script with appropriate command-line arguments to evaluate a DenseNet model.
    Example:
        python evaluate_densenet.py --variant attention --checkpoint ./checkpoints/densenet_attention_best.pth
"""
...
def evaluate(model, dataloader, criterion, device, num_classes):
    ...
    Parameters:
        model (nn.Module): The DenseNet model to evaluate.
    ...
    Returns:
        tuple: (epoch_loss, epoch_accuracy, epoch_auc)
            - epoch_loss (float): Average loss over the validation dataset.
            - epoch_accuracy (float): Overall accuracy.
            - epoch_auc (Tensor): AUROC score for each class.
    """
    model.eval()
    running_loss = 0.0
...
def main():
    ...
    Parses command-line arguments, instantiates the corresponding DenseNet variant,
    loads the model checkpoint, prepares the validation dataset and dataloader,
    and computes evaluation metrics (loss, accuracy, and AUROC per class). Finally,
    prints out the results using disease names from the CheXpert dataset.
    """
    parser = argparse.ArgumentParser(description="Evaluate DenseNet variant on CheXpert validation dataset")
    ...
    # Instantiate the appropriate DenseNet variant and set up image transforms.
    ...
    # Display AUROC for each disease.
    disease_names = ChexpertSmall.attr_names  # ['Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 'Pleural Effusion']
